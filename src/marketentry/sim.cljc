...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MKD, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up public-charges-arrears, no deferral) ==")
    ...
    (println "== filing/submit eng-4 (public-charges-arrears, no deferral approved -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up embs-unverified) ==")
    ...
    (println "== filing/submit eng-5 (embs-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (arrears WITH approved+current deferral -- cured) ==")
    ...
    (println "== filing/submit eng-6 (deferral approved AND currently paid -> governor-clean, still escalates for actuation) ==")
    (let [r (exec-op actor "t13" {:op :filing/submit :subject "eng-6"} operator)]
      (println r)
      (println (approve! actor "t13")))

    (println "== jurisdiction/assess eng-7 (arrears WITH approved-but-NOT-current deferral -- still violates, proves the conjunction) ==")
    ...
    (println "== filing/submit eng-7 (approved but not currently paid -> STILL HARD hold, approval alone does not cure) ==")
