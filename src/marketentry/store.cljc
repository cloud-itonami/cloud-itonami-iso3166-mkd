(ns marketentry.store
  "SSoT for the North Macedonia (MKD) market-entry compliance actor,
...
(defn demo-data
  ...
  `:annual-turnover` / `:tax-arrears-amount` are МКД (Macedonian denar),
  last finished financial year -- ground truth for the flagship
  `public-charges-arrears-violation?` check (ЗЈН Член 88(2)(a)). Unlike
  a sibling jurisdiction's single boolean 'reprogram confirmed' flag,
  North Macedonia's own statute requires BOTH
  `:tax-deferred-payment-approved?` AND `:tax-deferred-payment-current?`
  to cure arrears -- eng-6/eng-7 below exist SPECIFICALLY to exercise
  that conjunction (approved+current -> cured; approved-but-not-current
  -> still violates). `:requires-embs?`/`:embs-verified?` stand in for
  the ЕМБС (Central Registry company registration number, ЗТД Член
  10(3)/14) a business must hold and have verified before e-nabavki.gov.mk
  (ЕСЈН) portal participation."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Кита Фабрикација ДООЕЛ" :portal "ЕСЈН"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :annual-turnover 2000000 :tax-arrears-amount 0
             :tax-deferred-payment-approved? false :tax-deferred-payment-current? false
             :requires-embs? true :embs-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "ЕСЈН"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :annual-turnover 2000000 :tax-arrears-amount 0
             :tax-deferred-payment-approved? false :tax-deferred-payment-current? false
    ...
    "eng-3" {:id "eng-3" :operator "Минами Системи ДОО" :portal "ЕСЈН"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}
    "eng-4" {:id "eng-4" :operator "Хигаши Извоз ДОО" :portal "ЕСЈН"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}
    "eng-5" {:id "eng-5" :operator "Ниши Логистика ДОО" :portal "ЕСЈН"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :annual-turnover 2000000 :tax-arrears-amount 0
             :tax-deferred-payment-approved? false :tax-deferred-payment-current? false
             :requires-embs? true :embs-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}
    "eng-6" {:id "eng-6" :operator "Средина Граѓанска Технологија ДОО" :portal "ЕСЈН"
             ...
             :tax-deferred-payment-approved? true :tax-deferred-payment-current? true
             :requires-embs? true :embs-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}
    "eng-7" {:id "eng-7" :operator "Запад Трговија ДООЕЛ" :portal "ЕСЈН"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :annual-turnover 1200000 :tax-arrears-amount 8000
             :tax-deferred-payment-approved? true :tax-deferred-payment-current? false
             :requires-embs? true :embs-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MKD" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               annual-turnover tax-arrears-amount
                               tax-deferred-payment-approved? tax-deferred-payment-current?
                               requires-embs? embs-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? tax-deferred-payment-approved?) (assoc :engagement/tax-deferred-payment-approved? tax-deferred-payment-approved?)
    (some? tax-deferred-payment-current?) (assoc :engagement/tax-deferred-payment-current? tax-deferred-payment-current?)
    (some? requires-embs?)                (assoc :engagement/requires-embs? requires-embs?)
    (some? embs-verified?)                (assoc :engagement/embs-verified? embs-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/annual-turnover :engagement/tax-arrears-amount
   :engagement/tax-deferred-payment-approved? :engagement/tax-deferred-payment-current?
   :engagement/requires-embs? :engagement/embs-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :tax-deferred-payment-approved? (boolean (:engagement/tax-deferred-payment-approved? m))
     :tax-deferred-payment-current? (boolean (:engagement/tax-deferred-payment-current? m))
     :requires-embs? (boolean (:engagement/requires-embs? m))
     :embs-verified? (boolean (:engagement/embs-verified? m))
