  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Кита Фабрикација ДООЕЛ"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Кита Фабрикација ДООЕЛ" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest public-charges-arrears-with-no-deferral-is-held-and-unoverridable
  (testing "unpaid public charges with NO deferred-payment arrangement at all -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:public-charges-arrears-uncured} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest public-charges-arrears-approved-but-not-current-still-held
  (testing "the flagship check's CONJUNCTION: deferral APPROVED but NOT currently paid regularly -> STILL HARD hold, approval alone does not cure (ЗЈН Член 88(2)(a))"
    (let [[db actor] (fresh)
...
(deftest embs-unverified-is-held-and-unoverridable
  (testing "unverified ЕМБС when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:embs-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest public-charges-arrears-approved-and-current-does-not-block-submit
  (testing "a deferral that is BOTH approved AND currently paid regularly is accepted -- eng-6 has arrears but a valid conjunctive cure"
    (let [[db actor] (fresh)
